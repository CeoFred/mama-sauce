...
exports.getLoggedIn = function (req, res) {
  return successResponse__(res, { profile: req.user.profile, userId: req.user.userId });
};
...
exports.postLogin = (req, res, next) => {
  ...
  if (validationErrors.length) {
    return ErrorResponse(res, validationErrors);
  }
  ...
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      return ErrorResponse(res, info);
    }
    req.logIn(user, (err) => {
      if (err) { return next(err); }
      return successResponse__(res, 'Success! You are logged in.');
    });
  })(req, res, next);
};
...
exports.logout = (req, res) => {
  ...
  req.session.destroy((err) => {
    ...
    req.user = null;
    return successResponse__(res, 'Logged Out');
  });
};
...
exports.postSignup = (req, res, next) => {
  ...
  if ((!req.body.firstname || req.body.firstname.length === 0)) validationErrors.push({ msg: 'First Name is required' });
  if ((!req.body.lastname || req.body.lastname.length === 0)) validationErrors.push({ msg: 'Last Name is required' });
  if ((!req.body.mobile) || req.body.mobile.length === 0) validationErrors.push({ msg: 'Phone Number is required' });
  ...
  User.findOne({ email: req.body.email }, (err, existingUser) => {
    if (err) { return next(err); }
    if (existingUser) {
      return ErrorResponse(res, 'Account with that email address already exists.');
    }
    user.save((err) => {
      if (err) { return next(err); }
      req.logIn(user, (err) => {
        ...
        return successResponse__(res, 'Sigup Successful', 201);
      });
    });
  });
};
...
exports.postUpdatePassword = (req, res, next) => {
  ...
  User.findById(req.user.id, (err, user) => {
    ...
    user.save((err) => {
      if (err) { return next(err); }
      return successResponse__(res, { msg: 'Password reset success' }, 200);
    });
  });
};
